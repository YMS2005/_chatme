...
public class Login_Controller extends Application implements Initializable
{
    ...
    void Check_Validate()
    {
        ...
        if (String.valueOf(Math.abs(IDPT)).length() < 5)
        {
            ...
            {
                Alert_ERR("uh oh you entered less than 5 digits, make sure you enter the right number before proceeding.");
            }
        }
        else if (IDPT > 65535)
        {
    ...
    public void connect()
    {
        try {
            ...
            chatStage.setTitle("ChatME - Main Chat");
            chatStage.setScene(scene);
            chatStage.show();

            // Close the login window
            Stage loginStage = (Stage) IDtx.getScene().getWindow();
            loginStage.close();

            // Delay startConnection so Main_Controller finishes initializing its static fields first
            Platform.runLater(Client_X::startConnection);
